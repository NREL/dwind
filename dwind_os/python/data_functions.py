...
def create_scenario_results_folder(input_scenario, scen_name, scenario_names, out_dir, dup_n=0):
    ...
    **dup_n** : 'int'
    ...
    if scen_name in scenario_names:
        logger.info("Warning: Scenario name {0} is a duplicate. Renaming to {1}_{2}".format(scen_name, scen_name, dup_n))
...
def get_bass_params(con, schema):
    ...
    sql = """SELECT state_abbr,
             ...
             SELECT state_abbr,
                    ...
                    sector_abbr,
                    tech
             FROM {schema}.input_wind_bass_params;""".format(**inputs)
...
def get_annual_inflation(con, schema):
    ...
    inputs = locals().copy()
    sql = '''SELECT *
             FROM diffusion_shared.input_main_market_inflation;'''
    df = pd.read_sql(sql, con)
...
def get_selected_scenario(con, schema):
    sql = "SELECT * FROM diffusion_shared.input_main_nem_selected_scenario;"
    df = pd.read_sql(sql, con, coerce_float=False)
    value = df['val'][0]
