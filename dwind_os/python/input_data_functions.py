...
def import_agent_file(scenario_settings, con, cur, engine, model_settings, agent_file_status, input_name):
    ...
    **solar_agents** : 'Class'
    ...
    if agent_file_status == 'Use pre-generated Agents':
        ...
        agents_df = pd.read_pickle(os.path.join(input_agent_dir, scenario_userdefined_value+'.pkl'))
        ...
        cols = ['customers_in_bin_initial','load_kwh_per_customer_in_bin_initial','load_kwh_in_bin_initial',
                'max_demand_kw','avg_monthly_kwh','cap_cost_multiplier','developable_roof_sqft','pct_of_bldgs_developable']
        ...
        if scenario_settings.techs in [['wind']]:
            ingest_agent_core_attributes(agents.df, con, cur, engine, schema, model_settings.role, scenario_settings.sectors, scenario_settings.techs)
...
def process_elec_price_trajectories(elec_price_traj):
    ...
    **elec_price_change_traj** : 'pd.df'
    ...
    # The raw ReEDS prices are in 2018$ year
    # The values are normalized to year 2018
    ...
    for sector_abbr, sector in sectors.items():
        for tech in techs:
            ...
            sql = """SELECT a.agent_id,
                            ...
                            a.cap_cost_multiplier_{tech} as cap_cost_multiplier,
                            -- solar
                            a.solar_re_9809_gid,
                            a.tilt,
                            a.azimuth,
                            ...
                            a.j,
                            a.cf_bin,
                            a.min_allowable_blade_height_m,
                            a.max_allowable_blade_height_m,
                            -- replicate for each sector and tech
                            '{sector_abbr}'::CHARACTER VARYING(3) as sector_abbr,
                            '{sector}'::TEXT as sector,
                            '{tech}'::varchar(5) as tech
                    FROM {schema}.agent_core_attributes_{sector_abbr} a """.format(**inputs)
            sql_list.append(sql)

    inputs['sql_body'] = ' UNION ALL '.join(sql_list)
    sql = """DROP VIEW IF EXISTS {schema}.agent_core_attributes_all;
             CREATE VIEW {schema}.agent_core_attributes_all AS
             {sql_body};""".format(**inputs)
    cur.execute(sql)
    con.commit()
...
def ingest_agent_core_attributes(dataframe, con, cur, engine, schema, role, sectors, techs):
    
    dataframe = dataframe.reset_index()
    
    dataframe.rename(columns={'ba':'pca_reg',
                              'customers_in_bin_initial':'customers_in_bin', 
                              'load_kwh_per_customer_in_bin_initial':'load_kwh_per_customer_in_bin',
                              'load_kwh_in_bin_initial':'load_kwh_in_bin'}, inplace=True)
    
    core_cols = ['agent_id','pgid','tract_id_alias','county_id','bin_id','state_abbr','census_division_abbr','pca_reg','reeds_reg',
                 'customers_in_bin','load_kwh_per_customer_in_bin','load_kwh_in_bin','max_demand_kw','avg_monthly_kwh',
                 'crb_model','hdf_load_index','owner_occupancy_status','solar_re_9809_gid','tilt','azimuth',
                 'developable_roof_sqft','pct_of_bldgs_developable','bldg_size_class','i','j','cf_bin',
                 'min_allowable_blade_height_m','max_allowable_blade_height_m','sector_abbr','sector','tech']
    
    engine_con = engine.connect()
    for sector_abbr, sector in sectors.items():
        for tech in techs:
            
            cap_cost_attribute = 'cap_cost_multiplier_{}'.format(tech)
            dataframe.rename(columns={'cap_cost_multiplier':cap_cost_attribute}, inplace=True)
            core_cols += [cap_cost_attribute]
        
            name = 'agent_core_attributes_{}'.format(sector_abbr)
            dataframe.loc[dataframe['sector_abbr'] == sector_abbr][core_cols].to_sql(name, engine, schema=schema, index=False, if_exists='replace')
            
            sql = 'ALTER TABLE {}."{}" OWNER to "{}"'.format(schema, name, role)
            engine_con.execute(sql)
...
#%%
@decorators.fn_timer(logger=logger, tab_level=2, prefix='')
def process_turbine_configurations(engine, schema, role, sectors):
    
    engine_con = engine.connect()

    wind_tables = {}
    for input_name in ['wind_property_setbacks', 'wind_canopy_clearance', 'wind_allowable_turbine_sizes', 'wind_tech_performance']:
        table_name = "input_" + input_name + "_user_defined"
        scenario_name = get_userdefined_scenario_settings(schema, table_name, engine)
        scenario_value = scenario_name['val'].values[0]
        
        wind_tables[input_name] = scenario_value
    
    for sector_abbr, sector in sectors.items():

        allowable_turbines_sql = '''
                 DROP TABLE IF EXISTS {schema}.agent_allowable_turbines_lkup_{sector_abbr};
                 CREATE TABLE {schema}.agent_allowable_turbines_lkup_{sector_abbr} AS
                 WITH wind_siting_settings AS(
                     SELECT
                         a.blade_height_setback_factor,
                         a.required_parcel_size_cap_acres,
                         b.canopy_clearance_rotor_factor,
                         b.canopy_clearance_static_adder_m,
                         b.canopy_pct_requiring_clearance
                     FROM diffusion_shared."{wind_property_setbacks}" a
                     CROSS JOIN diffusion_shared."{wind_canopy_clearance}" b
                 ),
                 wind_siting_turbine_sizes AS(
                     SELECT
                         a.turbine_size_kw,
                         a.rotor_radius_m,
                         b.turbine_height_m,
                         b.turbine_height_m::numeric - a.rotor_radius_m * c.canopy_clearance_rotor_factor AS effective_min_blade_height_m,
                         b.turbine_height_m::numeric + a.rotor_radius_m AS effective_max_blade_height_m
                     FROM diffusion_shared.turbine_size_to_rotor_radius_lkup a
                     LEFT JOIN diffusion_shared."{wind_allowable_turbine_sizes}" b
                         ON a.turbine_size_kw = b.turbine_size_kw
                     CROSS JOIN wind_siting_settings c
                     WHERE b.allowed = true
                 )
                 ...
                 LEFT JOIN wind_siting_turbine_sizes b
                     ON b.effective_min_blade_height_m >= a.min_allowable_blade_height_m::numeric
                     AND b.effective_max_blade_height_m <= a.max_allowable_blade_height_m;'''.format(**wind_tables, schema=schema, sector_abbr=sector_abbr)
        engine_con.execute(allowable_turbines_sql)
        
        tbl_name = 'agent_allowable_turbines_lkup_{}'.format(sector_abbr)
        alter_sql = 'ALTER TABLE {}."{}" OWNER to "{}"'.format(schema, tbl_name, role)
        engine_con.execute(alter_sql)
        
        
    wind_performance_improvements_sql = '''
             DROP VIEW IF EXISTS {schema}.wind_performance_power_curve_transitions;
             CREATE OR REPLACE VIEW {schema}.wind_performance_power_curve_transitions AS
             WITH a AS (
                 SELECT
                     x.year,
                     x.turbine_size_kw,
                     CASE
                         WHEN x.perf_improvement_factor >= 0::numeric AND x.perf_improvement_factor < 0.1 THEN ARRAY[0::numeric, 0.1]
                         WHEN x.perf_improvement_factor >= 0.1 AND x.perf_improvement_factor < 0.25 THEN ARRAY[0.1, 0.25]
                         WHEN x.perf_improvement_factor >= 0.25 THEN ARRAY[0.25, 0.25]
                         ELSE NULL::numeric[]
                     END AS perf_improvement_factor_bounds,
                     CASE
                         WHEN x.perf_improvement_factor >= 0::numeric AND x.perf_improvement_factor < 0.1 THEN x.perf_improvement_factor / 0.1
                         WHEN x.perf_improvement_factor >= 0.1 AND x.perf_improvement_factor < 0.25 THEN (x.perf_improvement_factor - 0.1) / (0.25 - 0.1)
                         WHEN x.perf_improvement_factor >= 0.25 THEN 0::numeric
                         ELSE NULL::numeric
                     END AS interp_factor
                 FROM diffusion_shared."{wind_tech_performance}" x
             ), b AS (
                 SELECT
                     a.year,
                     a.turbine_size_kw,
                     a.perf_improvement_factor_bounds[1] AS perf_improvement_factor_1,
                     a.perf_improvement_factor_bounds[2] AS perf_improvement_factor_2,
                     a.interp_factor
                 FROM a
             )
             SELECT
                 ...
                 b.interp_factor
             FROM b
             LEFT JOIN diffusion_shared.wind_performance_turbine_size_classes c
                 ON b.turbine_size_kw = c.turbine_size_kw
             LEFT JOIN diffusion_shared.power_curve_lkup d
                 ON c.size_class = d.size_class AND b.perf_improvement_factor_1 = d.perf_improvement_factor
             LEFT JOIN diffusion_shared.power_curve_lkup e
                 ON c.size_class = e.size_class AND b.perf_improvement_factor_2 = e.perf_improvement_factor
             ORDER BY b.year, b.turbine_size_kw;'''.format(**wind_tables, schema=schema, sector_abbr=sector_abbr)
    engine_con.execute(wind_performance_improvements_sql)
    
    tbl_name = 'wind_performance_power_curve_transitions'
    alter_sql = 'ALTER TABLE {}."{}" OWNER to "{}"'.format(schema, tbl_name, role)
    engine_con.execute(alter_sql)
