...
class Agents(object):
    ...
    def run_with_runtime_tests(self, how_to_apply, func, func_args=None, cores=None, **kwargs):
        ...
        duplicated_columns = ['_x' in c for c in post_columns] #worry about this if we ever have a column with _x in it
        new_columns = [c for c in post_columns if c not in initial_columns]
        post_dtypes = list(results_df[initial_columns].dtypes) #not checking for new columns
