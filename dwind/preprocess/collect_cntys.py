...
def collect_cntys(state, app):
    p_cntys = os.path.join(
        "/projects/dwind/data/counties_by_state", 
        f"{state}.csv"
    )
    df = pd.read_csv(p_cntys, dtype=str)
    
    p_agents = '/projects/dwind/agents/'
    agents = []
    
    for _, row in df.iterrows():
        cnty = row['county_file']
        ...
        p = os.path.join(p_agents, state + '_' + cnty)
        if os.path.exists(p):
            ...
            if os.path.exists(p):
                ...
                agent_df['county_file'] = cnty
                agent_df['fips_code'] = fips
                
                # fix county_id
                cnty_f = '/projects/dwind/configs/costs/dgen_county_fips_mapping.csv'
                cnty_df = pd.read_csv(cnty_f, dtype={'fips_code': str})
                
                cnty_df = cnty_df[['fips_code', 'county_id']]
                cnty_df['fips_code'] = cnty_df['fips_code'].astype(str)
...
def collect_cntys_by_state(state):
    p_agents = '/projects/dwind/agents/'
    ...
    agents_fom_df = collect_cntys(state, 'fom')
    ...
    agents_btm_df = collect_cntys(state, 'btm')
